<template>
  <div class="border border-slate-700/50 rounded-lg overflow-hidden bg-slate-900/50">
    ...
    <div v-if="expanded && !loading" class="relative bg-slate-950/50">
      ...
      <!-- Split View -->
      <div v-if="viewMode === 'split'" class="overflow-auto text-xs font-mono" ref="diffContainer">
        <table class="w-full border-collapse">
          <tbody>
            <template v-for="(hunk, hunkIndex) in hunks" :key="hunkIndex">
              <!-- Hunk Header -->
              <tr class="bg-slate-800/50">
                <td colspan="6" class="px-3 py-1.5 text-slate-400 text-xs border-y border-slate-700/50">
              ...
              <template v-for="(line, lineIndex) in getVisibleLines(hunk.lines)" :key="`${hunkIndex}-${lineIndex}`">
                ...
                >
                  <!-- Old Side -->
                  <td class="px-2 py-0.5 text-slate-500 text-right select-none w-10 border-r border-slate-700/50 bg-slate-900/50">
                    {{ line.type !== 'add' ? line.oldLineNumber : '' }}
                  </td>
                  <td 
                    :class="[
                      'px-3 py-0.5 font-mono text-sm w-1/2',
                  ...
                  <!-- New Side -->
                  <td class="px-2 py-0.5 text-slate-500 text-right select-none w-10 border-x border-slate-700/50 bg-slate-900/50">
                    {{ line.type !== 'delete' ? line.newLineNumber : '' }}
                  </td>
                  <td class="px-1 w-6 bg-slate-900/50">
                    <button
                  ...
                  >
                    ...
                    <code 
                      ...
                      :class="line.type === 'add' ? 'text-green-300' : 'text-slate-300'"
                      v-html="highlightSyntax(line.content)"
                    />
                  </td>
                </tr>

                <!-- Comment Form & Existing Comments (spans both sides) -->
                <tr v-if="commentingLine === line.newLineNumber || getCommentsForLine(line.newLineNumber).length > 0">
                  <td colspan="6" class="p-0 bg-slate-800/50">
                    <!-- Comment Form -->
                    ...
                    <div v-if="getCommentsForLine(line.newLineNumber).length > 0" class="p-3 space-y-2 border-t border-slate-700/50">
                      <div
                        ...
                        :key="comment.id"
                        class="flex gap-2 p-2 bg-slate-900/80 rounded border border-slate-700/50"
                      >
                        <img
                        ...
                        <div class="flex-1 min-w-0">
                          ...
                          <p class="text-xs text-slate-300 leading-relaxed">{{ comment.body }}</p>
                        </div>
                      </div>
                    </div>
                  </td>
      ...
      <!-- Unified View -->
      <div v-else class="overflow-auto text-xs font-mono" ref="diffContainer">
        <table class="w-full border-collapse">
          <tbody>
            <template v-for="(hunk, hunkIndex) in hunks" :key="hunkIndex">
              <tr class="bg-slate-800/50">
                ...
                >
                  ...
                  <td
                    :class="[
                      ...
                      {
                        'text-green-300 bg-green-500/5': line.type === 'add',
                        'text-red-300 bg-red-500/5': line.type === 'delete',
                        'text-slate-300 bg-slate-900/30': line.type === 'context',
                      }
                    ]"
                  >
                    ...
                    <code v-html="highlightSyntax(line.content)" />
                  </td>
                </tr>
